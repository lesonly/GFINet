...
class Data(Dataset):
    def __init__(self, cfg):
        with open(os.path.join(cfg.datapath, cfg.mode + '.txt'), 'r') as lines:
            ...
            for line in lines:
                ...
                edgepath = os.path.join(cfg.datapath, 'mask_edge', line.strip() + '.png')
                regionpath = os.path.join(cfg.datapath, 'mask_region', line.strip() + '.png')
                self.samples.append([imagepath, maskpath, edgepath,regionpath])
    ...
    def __getitem__(self, idx):
        imagepath, maskpath, edgepath,regionpath = self.samples[idx]
        ...
        H, W, C = mask.shape
        image, mask, edge,region = self.transform(image, mask, edge,region)
        return image, mask, edge,region, (H, W), maskpath.split('/')[-1]
